...
load P2veri.mat
x1=veri(:,3);%1st input for training
x2=veri(:,2);%2nd input for training
x3=veri(:,1);%3th input for training
x=[x1 x2 x3];
y=veri(:,4);%output for training 

%% prepare the inputs/output for testing
load P2veritest.mat
x1t=veri(:,3);%1st input for testing 
x2t=veri(:,2);%2nd input for testing
x3t=veri(:,1);%3th input for testing
xt=[x1t x2t x3t]; %input for testing
yt=veri(:,4);; %output for testig 
%% Plotting
...
teta=[];%parameter array/matr ix

Xe= [x ones(size(x1))]; %%% extended input 
%% membership funtions deffination
%for the first input ,u(k)
x1_array=-1:0.1:1;
    A1 = gaussmf(x1_array, [0.4 -0.5]);
    A2 = gaussmf(x1_array, [0.4 0.5]);
    figure(2)
   subplot(3,1,1); plot(x1_array, A1, x1_array, A2); 
   xlabel('u(k)');
   title('MFs for u(k) input');

%for the second input ,y(k-1)
x2_array=-2:0.1:2;
    B1 = gaussmf(x2_array, [0.8 -1]);
    B2 = gaussmf(x2_array, [0.8 1]);
   subplot(3,1,2); plot(x2_array, B1, x2_array, B2); 
   %subplot divides the current figure into an m-by-n grid and creates axes in the position specified 
   xlabel('y(k)');
   title('MFs for y(k) input');
   
%for the third input ,y(k-2)
x3_array=-2:0.1:2;
    C1 = gaussmf(x3_array, [0.8 -1]);
    C2 = gaussmf(x3_array, [0.8 1]);
...
for n= 1:length(x1)
    ...
    w1=gaussmf(x1(n), [0.4 -0.5])*gaussmf(x2(n), [0.8 -1])*gaussmf(x3(n), [0.8 -1]);%A1 B1 C1
    w2=gaussmf(x1(n), [0.4 -0.5])*gaussmf(x2(n), [0.8 -1])*gaussmf(x3(n), [0.8 1]); %A1 B1 C2
    w3=gaussmf(x1(n), [0.4 -0.5])*gaussmf(x2(n), [0.8 1])*gaussmf(x3(n), [0.8 -1]); %A1 B2 C1
    w4=gaussmf(x1(n), [0.4 -0.5])*gaussmf(x2(n), [0.8 1])*gaussmf(x3(n), [0.8 1]); %A1 B2 C2
    w5=gaussmf(x1(n), [0.4 0.5])*gaussmf(x2(n), [0.8 -1])*gaussmf(x3(n), [0.8 -1]); %A2 B1 C1
    w6=gaussmf(x1(n), [0.4 0.5])*gaussmf(x2(n), [0.8 -1])*gaussmf(x3(n), [0.8 1]); %A2 B1 C2
    w7=gaussmf(x1(n), [0.4 0.5])*gaussmf(x2(n), [0.8 1])*gaussmf(x3(n), [0.8 -1]);%A2 B2 C1
    w8=gaussmf(x1(n), [0.4 0.5])*gaussmf(x2(n), [0.8 1])*gaussmf(x3(n), [0.8 1]); %A2 B2 C2
...
%% PERFORMANCE FOR TRAINING DATA with founded rule parameter 
    ...
    for i= 1:length(x1)
        ...
        w1=gaussmf(x1(i), [0.4 -0.5])*gaussmf(x2(i), [0.8 -1])*gaussmf(x3(i), [0.8 -1]);%A1 B1 C1
        w2=gaussmf(x1(i), [0.4 -0.5])*gaussmf(x2(i), [0.8 -1])*gaussmf(x3(i), [0.8 1]); %A1 B1 C2
        w3=gaussmf(x1(i), [0.4 -0.5])*gaussmf(x2(i), [0.8 1])*gaussmf(x3(i), [0.8 -1]); %A1 B2 C1
        w4=gaussmf(x1(i), [0.4 -0.5])*gaussmf(x2(i), [0.8 1])*gaussmf(x3(i), [0.8 1]); %A1 B2 C2
        w5=gaussmf(x1(i), [0.4 0.5])*gaussmf(x2(i), [0.8 -1])*gaussmf(x3(i), [0.8 -1]); %A2 B1 C1
        w6=gaussmf(x1(i), [0.4 0.5])*gaussmf(x2(i), [0.8 -1])*gaussmf(x3(i), [0.8 1]); %A2 B1 C2
        w7=gaussmf(x1(i), [0.4 0.5])*gaussmf(x2(i), [0.8 1])*gaussmf(x3(i), [0.8 -1]);%A2 B2 C1
        w8=gaussmf(x1(i), [0.4 0.5])*gaussmf(x2(i), [0.8 1])*gaussmf(x3(i), [0.8 1]); %A2 B2 C2
...
%% PERFORMANCE FOR TESTING DATA with founded rule parameter 
    ...
    for i= 1:length(x1t)
        ...
        w1=gaussmf(x1t(i), [0.4 -0.5])*gaussmf(x2t(i), [0.8 -1])*gaussmf(x3t(i), [0.8 -1]);%A1 B1 C1
        w2=gaussmf(x1t(i), [0.4 -0.5])*gaussmf(x2t(i), [0.8 -1])*gaussmf(x3t(i), [0.8 1]); %A1 B1 C2
        w3=gaussmf(x1t(i), [0.4 -0.5])*gaussmf(x2t(i), [0.8 1])*gaussmf(x3t(i), [0.8 -1]); %A1 B2 C1
        w4=gaussmf(x1t(i), [0.4 -0.5])*gaussmf(x2t(i), [0.8 1])*gaussmf(x3t(i), [0.8 1]); %A1 B2 C2
        w5=gaussmf(x1t(i), [0.4 0.5])*gaussmf(x2t(i), [0.8 -1])*gaussmf(x3t(i), [0.8 -1]); %A2 B1 C1
        w6=gaussmf(x1t(i), [0.4 0.5])*gaussmf(x2t(i), [0.8 -1])*gaussmf(x3t(i), [0.8 1]); %A2 B1 C2
        w7=gaussmf(x1t(i), [0.4 0.5])*gaussmf(x2t(i), [0.8 1])*gaussmf(x3t(i), [0.8 -1]);%A2 B2 C1
        w8=gaussmf(x1t(i), [0.4 0.5])*gaussmf(x2t(i), [0.8 1])*gaussmf(x3t(i), [0.8 1]); %A2 B2 C2
